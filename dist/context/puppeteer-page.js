...
class PuppeteerPage {
     ...
     */
    async load(url, language, reload = false) {
        if (true === reload && this._page instanceof puppeteer_1.Page) {
            await this.reload();
        }
        else {
    ...
    async content() {
        ...
        const dom = new jsdom_1.JSDOM(content, { url: this.url() });
        return dom;
    }
    /**
     ...
     */
    captchaCFTurnstileProcessor() {
        const getConfig = async () => {
            let node;
            const findCaptchaFrame = async () => {
                const hasCaptchaScriptTag = await this._page.$(`script[src*="/turnstile/v0/api.js"]`);
                if (hasCaptchaScriptTag instanceof puppeteer_1.JSHandle) {
                    node = await this._page.$(`iframe[src*="challenges.cloudflare.com"][src*="/turnstile"]`);
                }
            };
            const buildConfig = async () => {
                const contentFrame = await node.contentFrame();
                const cfChlOpt = await contentFrame.evaluate(async () => {
            ...
            if (node instanceof puppeteer_1.JSHandle) {
                return await buildConfig();
            }
        };
        const submit = async (token, callbackName) => {
            await this._page.evaluate(async (token, _callbackName) => {
     ...
     * and submit captcha like user do.
     */
    captchaReCaptchaProcessor() {
        const getConfig = async () => {
            let node;
            const findCaptchaNode = async () => {
            ...
            const buildConfig = async () => {
                const sitekey = await node.evaluate((element) => {
                    return element.dataset.sitekey;
                });
                const callback = await node.evaluate((element) => {
     ...
     */
    async traceOutgoingIP() {
        const callback = async (source) => {
            try {
